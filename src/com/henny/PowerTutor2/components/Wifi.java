...
public class Wifi extends PowerComponent {
		...
		public void writeLogDataInfo(OutputStreamWriter out) throws IOException {
			...
			if (wifiOn) {
				res.append("Wifi-packets ").append((long) Math.round(packets))
						...
						.append("\nWifi-uplink ")
						.append((long) Math.round(uplinkRate))
						.append("\nWifi-speed ")
						.append((long) Math.round(linkSpeed))
						.append("\nWifi-state ")
						.append(Wifi.POWER_STATE_NAMES[powerState])
						.append("\n");
			}
	...
	public Wifi(Context context, PhoneConstants phoneConstants) {
		...
		changeWifiDirectory("/sys/devices/virtual/net/");

		uidStatsFolder = new File("/proc/uid_stat");
	}

	private void changeWifiDirectory(String prefix) {
		transPacketsFile = prefix + interfaceName + "/statistics/tx_packets";
	...
	public IterationData calculateIteration(long iteration) {
		 ...
		 * structure from WifiManager is a little bit expensive. This isn't
		 * really something that is likely to change very frequently anyway.
		 */
		...
		if (lastUids != null)
			for (int uid : lastUids) {
				...
				try {
					...
					if (uidState == null) {
						uidState = new WifiStateKeeper(
								phoneConstants.wifiHighLowTransition(),
								phoneConstants.wifiLowHighTransition());
					...
					receiveBytes = sysInfo.readLongFromFile("/proc/uid_stat/"
							+ uid + "/tcp_rcv");
					transmitBytes = sysInfo.readLongFromFile("/proc/uid_stat/"
							+ uid + "/tcp_snd");
					...
					if (receiveBytes == -1 || transmitBytes == -1) {
						...
						/*
						 * We only have information about bytes received but
						 * what we really want is the number of packets received
						 * so we just have to estimate it.
						 */
						long deltaTransmitBytes = transmitBytes
								- uidState.getTransmitBytes();
						long deltaReceiveBytes = receiveBytes
								- uidState.getReceiveBytes();
						long estimatedTransmitPackets = (long) Math
								.round(deltaTransmitBytes
										/ wifiState
												.getAverageTransmitPacketSize());
						long estimatedReceivePackets = (long) Math
								.round(deltaReceiveBytes
										/ wifiState
												.getAverageReceivePacketSize());
						if (deltaTransmitBytes > 0
								&& estimatedTransmitPackets == 0) {
							estimatedTransmitPackets = 1;
						}
						if (deltaReceiveBytes > 0
								&& estimatedReceivePackets == 0) {
							estimatedReceivePackets = 1;
						}

						boolean active = transmitBytes != uidState
								.getTransmitBytes()
								|| receiveBytes != uidState.getReceiveBytes();
						uidState.updateState(uidState.getTransmitPackets()
								+ estimatedTransmitPackets,
								uidState.getReceivePackets()
										+ estimatedReceivePackets,
								transmitBytes, receiveBytes);
						...
						if (active) {
							...
							uidData.init(uidState.getPackets(),
									...
									uidState.getUplinkRate(), linkSpeed,
									uidState.getPowerState());
	...
	private static class WifiStateKeeper {
		...
		public WifiStateKeeper(double highLowTransition,
				double lowHighTransition) {
